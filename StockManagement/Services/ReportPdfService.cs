...
public interface IReportPdfService
{
    ...
    Task<byte[]> GenerateStockCardReportAsync(int productId, DateTime? startDate, DateTime? endDate, string userName);
}
...
internal class ReportPdfService : IReportPdfService
{
    ...
    public async Task<byte[]> GenerateStockCardReportAsync(int productId, DateTime? startDate, DateTime? endDate, string userName)
    {
        // Get stock card data
        var stockCard = await _reportService.GetStockCardAsync(productId, startDate, endDate);
        ...
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                // Header
                page.Header().Element(header => ComposeStockCardReportHeader(header, stockCard, userName));

                // Content
                page.Content().Element(content => ComposeStockCardReportContent(content, stockCard));
    ...
    private static void ComposeStockCardReportHeader(IContainer container, StockCardViewModel stockCard, string userName)
    {
        container.Column(column =>
        {
            // Title
            column.Item().AlignCenter().Text("STOCK CARD REPORT")
                .FontSize(20)
                .Bold()
                .FontColor(Colors.Blue.Darken2);
            ...
            column.Item().Border(1).BorderColor(Colors.Grey.Medium).Padding(10).Column(detailsColumn =>
            {
                ...
                detailsColumn.Item().Row(row =>
                {
                    row.RelativeItem().Column(col =>
                    {
                        col.Item().Text("Product Code").FontSize(8).FontColor(Colors.Grey.Darken1);
                        col.Item().Text(stockCard.ProductCode ?? "-").FontSize(11).Bold();
                    });

                    row.RelativeItem().Column(col =>
                    {
                        col.Item().Text("Product Name").FontSize(8).FontColor(Colors.Grey.Darken1);
                        col.Item().Text(stockCard.ProductName).FontSize(11).Bold();
                    });

                    row.RelativeItem().Column(col =>
                    {
                        col.Item().Text("Current Stock").FontSize(8).FontColor(Colors.Grey.Darken1);
                        col.Item().Text(stockCard.CurrentStockLevel.ToString("N0")).FontSize(11).Bold()
                            .FontColor(Colors.Blue.Darken2);
                    });
                });
                ...
                detailsColumn.Item().PaddingTop(10).Row(row =>
                {
                    row.RelativeItem().Column(col =>
                    {
                        col.Item().Text("Period Start").FontSize(8).FontColor(Colors.Grey.Darken1);
                        col.Item().Text(stockCard.StartDate.ToShortDateString()).FontSize(11).Bold();
                    });

                    row.RelativeItem().Column(col =>
                    {
                        col.Item().Text("Period End").FontSize(8).FontColor(Colors.Grey.Darken1);
                        col.Item().Text(stockCard.EndDate.ToShortDateString()).FontSize(11).Bold();
                    });

                    row.RelativeItem().Column(col =>
                    {
                        col.Item().Text("Initial Balance").FontSize(8).FontColor(Colors.Grey.Darken1);
                        col.Item().Text(stockCard.InitialBalance.ToString("N0")).FontSize(11).Bold()
                            .FontColor(stockCard.InitialBalance == 0 ? Colors.Black :
                                      stockCard.InitialBalance > 0 ? Colors.Green.Darken2 : Colors.Red.Darken2);
                    });

                    row.RelativeItem().Column(col =>
                    {
                        col.Item().Text("Total Operations").FontSize(8).FontColor(Colors.Grey.Darken1);
                        col.Item().Text(stockCard.TotalRows.ToString("N0")).FontSize(11).Bold();
                    });
                });
            });
    ...
    private static void ComposeStockCardReportContent(IContainer container, StockCardViewModel stockCard)
    {
        // Calculate statistics
        var totalStockIn = stockCard.Rows.Where(r => r.Type == OperationType.StockIn).Sum(r => r.Quantity);
        var totalStockOut = stockCard.Rows.Where(r => r.Type == OperationType.StockOut).Sum(r => r.Quantity);
        var finalBalance = stockCard.Rows.Count != 0 ? stockCard.Rows.Last().Balance : stockCard.InitialBalance;

        container.Column(column =>
        {
            // Summary section
            if (stockCard.Rows.Count != 0)
            {
                column.Item().PaddingBottom(10).Row(summaryRow =>
                {
                    summaryRow.RelativeItem().Background(Colors.Green.Lighten4).Padding(8).Column(col =>
                    {
                        col.Item().Text("Total Stock In").FontSize(8).FontColor(Colors.Grey.Darken1);
                        col.Item().Text($"+{totalStockIn:N0}").FontSize(14).Bold().FontColor(Colors.Green.Darken2);
                    });

                    summaryRow.RelativeItem().Background(Colors.Red.Lighten4).Padding(8).Column(col =>
                    {
                        col.Item().Text("Total Stock Out").FontSize(8).FontColor(Colors.Grey.Darken1);
                        col.Item().Text($"-{totalStockOut:N0}").FontSize(14).Bold().FontColor(Colors.Red.Darken2);
                    });

                    summaryRow.RelativeItem().Background(Colors.Blue.Lighten4).Padding(8).Column(col =>
                    {
                        col.Item().Text("Final Balance").FontSize(8).FontColor(Colors.Grey.Darken1);
                        col.Item().Text(finalBalance.ToString("N0")).FontSize(14).Bold().FontColor(Colors.Blue.Darken2);
                    });

                    summaryRow.RelativeItem().Background(Colors.Grey.Lighten3).Padding(8).Column(col =>
                    {
                        col.Item().Text("Net Change").FontSize(8).FontColor(Colors.Grey.Darken1);
                        var netChange = finalBalance - stockCard.InitialBalance;
                        col.Item().Text($"{(netChange >= 0 ? "+" : "")}{netChange:N0}").FontSize(14).Bold()
            ...
            if (stockCard.Rows.Count != 0)
            {
                column.Item().Table(table =>
                {
                    ...
                    table.Header(header =>
                    {
                        ...
                        header.Cell().Element(HeaderCellStyle).Text("Supplier").Bold();
                    ...
                    // Rows
                    foreach (var row in stockCard.Rows)
                    {
                        var backgroundColor = row.Type == OperationType.StockIn
                            ? Colors.Green.Lighten4
                            : Colors.Red.Lighten4;

                        var quantityColor = row.Type == OperationType.StockIn
                            ? Colors.Green.Darken2
                            : Colors.Red.Darken2;

                        table.Cell().Element(container => RowCellStyle(container, backgroundColor))
                            .Text(row.Date.ToShortDateString());

                        table.Cell().Element(container => RowCellStyle(container, backgroundColor))
                            .Text(row.Type == OperationType.StockIn ? "IN" : "OUT")
                            .FontSize(9)
                            .Bold()
                        ...
                        table.Cell().Element(container => RowCellStyle(container, backgroundColor))
                            .AlignRight()
                            .Text($"{(row.Type == OperationType.StockIn ? "+" : "-")}{row.Quantity:N0}")
                            .FontColor(quantityColor);

                        table.Cell().Element(container => RowCellStyle(container, backgroundColor))
                            .AlignRight()
                            .Text(row.Balance.ToString("N0"))
                            .Bold();
                    }
            ...
            {
                column.Item().AlignCenter().PaddingVertical(20).Text("No operations found for this period")
                    .FontSize(12)
                    .FontColor(Colors.Grey.Darken1);
            }
            ...
            if (stockCard.Rows.Count != 0)
            {
                column.Item().PaddingTop(15).Row(legendRow =>
                {
                    legendRow.RelativeItem().Row(row =>
                    {
                        row.ConstantItem(15).Height(15).Background(Colors.Green.Lighten4);
                        row.ConstantItem(5);
                        row.AutoItem().Text("Stock In Operations").FontSize(8);
                    });
                    ...
                    legendRow.RelativeItem().Row(row =>
                    {
                        row.ConstantItem(15).Height(15).Background(Colors.Red.Lighten4);
                        row.ConstantItem(5);
                        row.AutoItem().Text("Stock Out Operations").FontSize(8);
                    });
                });
            }
        });
